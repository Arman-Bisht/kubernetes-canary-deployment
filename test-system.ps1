...
try {
    $health = Invoke-WebRequest -Uri "http://localhost/health/live" -UseBasicParsing
...
try {
    $ready = Invoke-WebRequest -Uri "http://localhost/health/ready" -UseBasicParsing
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost/" -Headers @{"canary"="true"} -UseBasicParsing
